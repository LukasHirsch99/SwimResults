...
{
  // 01.-05.08.2020
  let m = s.match(/^(?<firstDay>\d{2}).-(?<lastDay>\d{2}).(?<month>\d{2}).(?<year>\d{4})/)?.groups
  ...
  // 29.02.-01.03.2020
  m = s.match(/^(?<firstDay>\d{2}).(?<firstMonth>\d{2}).-(?<lastDay>\d{2}).(?<lastMonth>\d{2}).(?<year>\d{4})/)?.groups
  if (m)
  {
    return {
...
const insertClubs = async (meetId: number) =>
{
  ...
  const data = await axiod.get(`https://myresults.eu/de-DE/Meets/Today-Upcoming/${meetId}/Overview/Statistics`);
  const soup = new DOMParser().parseFromString(data.data, "text/html");
  const clubElements = soup?.getElementsByClassName("myresults_content_divtablerow");
  if (!clubElements)
    return;
  ...
  {
    const matches = clubElement.getElementsByTagName("a")[0].attributes.getNamedItem("href")?.value.match(/\d*$/)
    if (!matches)
    ...
    const flagLink = clubElement.getElementsByTagName("img")[0].attributes.getNamedItem("src")?.value
    const nationality = flagLink ? `https://myresults.eu${flagLink}` : undefined
    clubs.push({
      id: clubId,
      name: name,
...
{
  let data = await axiod.get(`https://myresults.eu/de-DE/Meets/Today-Upcoming/${meetId}/Overview`);
  let soup = new DOMParser().parseFromString(data.data, "text/html");
  if (!soup)
    return null

  const details = soup.getElementsByClassName("row myresults_content_divtablerow")
  if (!details)
    return null

  if (!details[6].innerText.includes("Austria"))
    return null

  const name = soup.getElementsByClassName("row myresults_content_divtablerow myresults_content_divtablerow_header")[0].innerText.trim()
  const imgTag = soup.getElementsByClassName("img-responsive center-block")[0]
  const image = imgTag ? "https://myresults.eu" + getAttr(imgTag, "src") : undefined
  const date = parseDate(details[3].childNodes[1].childNodes[0].textContent.trim())
  const deadline = format(parse(details[4].childNodes[1].childNodes[0].textContent.trim(), "dd.MM.yyyy HH:mm"), "yyyy-MM-dd HH:mm:ss")
  ...
  else
  {
    ...
    googlemapslink = soup?.getElementsByClassName("text-right")[0].getElementsByTagName("a")[0].attributes.getNamedItem("href")?.value
    const invLinks = soup?.getElementsByTagName("a").filter((v) => v.attributes.getNamedItem("href")?.value.includes(".pdf"))
    invLinks?.forEach((v) =>
      invitations.push(getAttr(v, "href"))
    )
  }
...
{
  if (updateRecent)
    console.log("Updating Recent Meets");
  else
    console.log("Updating Upcoming Meets");
    
  const data = await axiod.get(`https://myresults.eu/de-DE/Meets/${updateRecent ? "Recent" : "Today-Upcoming"}`);
  const soup = new DOMParser().parseFromString(data.data, "text/html");

  const meetLinks = soup?.getElementsByTagName("a").filter((v) => getAttr(v, "href").includes("Overview"));
  if (!meetLinks)
    throw new Error("Didn't find any new upcoming meets")
  ...
  for (const meetLink of meetLinks)
  {
    ...
    if (!matches)
      throw new Error("Couldn't parse meetId")
    ...
    console.log((await supabase.functions.invoke("UpdateSchedule", { body: { meetId: meetId } })).error)
  }
}
...
Deno.serve(async (req: Request) =>
{
  ...
  if (clubIdSetData == null)
    throw new Error("Couldn't get clubIdSet")
  else if (meetIdSetData == null)
    throw new Error("Couldn't get meetIdSet")
