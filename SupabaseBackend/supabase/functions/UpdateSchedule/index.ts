...
if (maxHeatIdData == null) throw new Error("Couldn't get maxHeatId");
else if (maxResultIdData == null) throw new Error("Couldn't get maxResultId");
else if (swimmerIdSetData == null) throw new Error("Couldn't get swimmerIds");
else if (clubIdSetData == null) throw new Error("Couldn't get clubIds");
...
const parseSessionInfo = (heading: string): SessionInfo => {
  ...
  if (!results) throw new Error("Invalid session string format");
...
const parseTime = (time: string) => {
  ...
  if (!time.includes(":")) return format(parse(time, "ss.SS"), "HH:mm:ss.SS");
...
const insertSwimmerFromStartOrResult = async (
  swimmerId: number,
  divElement: Element,
) => {
  console.log("Inserting new swimmer: " + swimmerId);
  ...
  swimmer.firstname = firstLetterUppercase(hrefs[0].innerText.split(" ")[1]);
  swimmer.clubid = Number.parseInt(matches[0]);
  if (!clubIdSet.includes(swimmer.clubid)) {
    const flagLink = divElement
    ...
    const club: Club = {
      id: swimmer.clubid,
      name: hrefs[1].innerText,
      nationality: nationality,
    };
    console.log("Inserting club: " + club.toString());
    clubIdSet.push(club.id);
    await insertInto("club", club);
  }

  const birthAndGender = divElement
    .getElementsByClassName("myresults_content_divtable_details")[0]
    .innerText.match(/\d+|[A-Z]/g);
  if (!birthAndGender) {
    throw new Error("No gender or birthyear information found");
  }
  ...
  await insertInto("swimmer", swimmer);
  // let { error } = await supabase.from("swimmer").insert(swimmer)
  // if (error) throw error;
  // await supabase.from("swimmer").insert(swimmer);
};

const insertStartInfo = async (
  meetId: number,
  startId: number,
  eventId: number,
) => {
  const data = await axiod.get(
    `https://myresults.eu/de-DE/Meets/Today-Upcoming/${meetId}/Starts/${startId}`,
  );
  const soup = new DOMParser().parseFromString(data.data, "text/html");
  const divElements = soup?.getElementsByClassName(
    "myresults_content_divtablerow",
  );
  if (!divElements) return;

  const startCnt = divElements.filter(
    (element) =>
      !element.classList.contains("myresults_content_divtablerow_header"),
  ).length;
  ...
  if (startCnt == dbStarts.count && heatCnt == dbHeats.count) return;
  ...
  for (const divElement of divElements) {
    ...
    if (divElement.classList.contains("myresults_content_divtablerow_header")) {
      ...
      const matches = getAttr(
        divElement.getElementsByTagName("a")[0],
        "href",
      ).match(/\d*$/);
      ...
      const startTime = parseTime(
        divElement.getElementsByClassName(
          "hidden-xs col-sm-2 col-md-1 text-right myresults_content_divtable_right",
        )[0].innerText,
      );
      const lane = Number.parseInt(
        divElement.getElementsByClassName("col-xs-1")[0].innerText,
      );
...
const insertResultInfo = async (
  meetId: number,
  msecmResultId: number,
  eventId: number,
) => {
  const data = await axiod.get(
    `https://myresults.eu/de-DE/Meets/Today-Upcoming/${meetId}/Results/${msecmResultId}`,
  );
  const soup = new DOMParser().parseFromString(data.data, "text/html");
  const divElements = soup?.getElementsByClassName(
    "myresults_content_divtablerow",
  ...
  const resultCnt = divElements.filter(
    (element) =>
      !element.classList.contains("myresults_content_divtablerow_header"),
  ).length;

  const { count: dbResultCnt } = await supabase
    .from("ageclass")
    .select("*, result!inner(*)", { count: "exact" })
    .eq("result.eventid", eventId);

  if (resultCnt == dbResultCnt) return;
  ...
  for (const divElement of divElements) {
    ...
    if (divElement.classList.contains("myresults_content_divtablerow_header")) {
      ...
      const resultInfoString = divElement
        .getElementsByClassName("myresults_content_divtable_points")[0]
        .innerText.trim();
      const resultInfo = evaluteRegex(
        /(?<timeToFirst>\+\d+\.\d+)|(?<finaPoints>\d+)|(?<additionalInfo>[\S]+)/g,
        resultInfoString,
      );
      ...
      if (dbResultId === undefined) {
        ...
        const finaPoints = finaPointsString
          ? Number.parseInt(finaPointsString)
          : undefined;
        const additionalInfo = resultInfo.get("additionalInfo");
        const splits = divElement
          .getElementsByClassName("myresults_content_divtable_details")[3]
          .innerText.trim();
...
const updateSchedule = async (meetId: number) => {
  const data = await axiod.get(
    "https://myresults.eu/de-DE/Meets/Today-Upcoming/" + meetId + "/Schedule",
  );
  const soup = new DOMParser().parseFromString(data.data, "text/html");
  const divElements = soup?.getElementsByClassName(
    "myresults_content_divtablerow",
  ...
  const eventCnt = divElements.filter(
    (event) =>
      !event.classList.contains("myresults_content_divtablerow_header"),
  ).length;
  ...
  const scheduleUpToDate =
    eventCnt == dbEvents.count && sessionCnt == dbSessions.count;
  ...
  for (const divElement of divElements) {
    ...
    if (divElement.classList.contains("myresults_content_divtablerow_header")) {
      ...
      if (scheduleUpToDate) {
        sessionId = dbSessions.data?.find(
          (session) =>
            session["displaynr"] == sessionInfo.displaynr &&
            session["day"] == sessionInfo.day,
        )["id"];
      } else {
      ...
      const eventInfo = parseEventInfo(
        divElement
          .getElementsByClassName("col-xs-6")[0]
          .childNodes[2].textContent.trim(),
      );
      if (!eventInfo.displaynr || !eventInfo.name) continue;

      let eventId: number;

      if (scheduleUpToDate) {
        eventId = dbEvents.data?.find(
          (event) =>
            event["sessionid"] == sessionId &&
            event["name"] == eventInfo.name &&
            event["displaynr"] == eventInfo.displaynr,
        )["id"];
      } else {
        const { data, error } = await supabase
          .from("event")
      ...
      const hrefs = divElement.getElementsByClassName(
        "myresults_content_link myresults_content_divtablecol",
      );

      if (!hrefs || !hrefs.length) continue;
...
await updateSchedule(2090);

Deno.serve(async (req) => {
  const { data: maxHeatIdData } = await supabase.rpc("maxheatid").select();
  const { data: maxResultIdData } = await supabase.rpc("maxresultid").select();
  const { data: swimmerIdSetData } = await supabase
    .from("swimmer")
    .select("id");
  const { data: clubIdSetData } = await supabase.from("club").select("id");
